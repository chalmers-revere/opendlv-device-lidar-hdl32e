...
// Calibration data for HDL32e.
const char *HDL32e_XML = R"(
...
void HDL32eDecoder::setupCalibration() noexcept {
    ...
    const std::string CALIBRATION(HDL32e_XML);
...
void HDL32eDecoder::index32sensorIDs() noexcept {
...
std::pair<std::vector<opendlv::proxy::PointCloudReading>, cluon::data::TimeStamp> HDL32eDecoder::decode(const std::string &data) noexcept {
            ...
            if (m_pointIndexCPC < MAX_POINT_SIZE) {
                ...
                for (uint8_t counter{0}; counter < 32; counter++) {
                    ...
                    if (m_pointIndexCPC >= MAX_POINT_SIZE) {
                        ...
                        std::cerr << "[lidar-hdl32e] More than 70,000 points?" << std::endl; 
                        break;
                    }
                }
            }
